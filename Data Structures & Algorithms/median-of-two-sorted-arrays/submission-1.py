class Solution:
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        while True:
            ...
            ALeft=l1[i] if i>=0 else float("-infinity")
            BLeft=l2[j] if j>=0 else float("-infinity")
            ARight=l1[i+1] if i+1<n1 else float("infinity")
            BRight=l2[j+1] if j+1<n2 else float("infinity")
            lMax=max(ALeft,BLeft)
            rmin=min(ARight,BRight)
